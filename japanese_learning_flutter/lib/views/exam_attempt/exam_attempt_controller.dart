...
class ExamAttemptController extends ChangeNotifier {
  ...
  Future<void> initExam() async {
    ...
    try {
      ...
      final attemptData = await _repository.startExam(1, examId);
      ...
      final rawParts = await _repository.getQuestions(examId);
      ...
      try {
        final examDetail = await _repository.getExamDetail(examId);
  ...
  Future<void> _syncAnswersToBackend() async {
    ...
    try {
      ...
      await _repository.autoSaveAnswer(_attemptId!, requests);
  ...
  Future<Map<String, dynamic>> submitExam() async {
    ...
    final response = await _repository.submitExam(_attemptId!, requests);
    return response;
  }
