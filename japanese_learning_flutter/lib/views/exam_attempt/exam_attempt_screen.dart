...
class _ExamAttemptScreenState extends ConsumerState<ExamAttemptScreen> {
  /// Shorthand để gọi action
  ExamAttemptNotifier get _notifier =>
      ref.read(examAttemptProvider(widget.examId).notifier);
  /// Shorthand để đọc state (Riverpod tự rebuild widget khi state thay đổi)
  ExamAttemptState get _state =>
      ref.watch(examAttemptProvider(widget.examId));
  ...
  void didChangeDependencies() {
    ...
    final seconds = ref.watch(
        examAttemptProvider(widget.examId).select((s) => s.secondsRemaining));
    if (seconds <= 0 && !_state.isLoading && _state.error == null) {
      WidgetsBinding.instance.addPostFrameCallback((_) => _submitExam());
    }
  }

  Future<void> _forceClearLocalData() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      final keys = prefs.getKeys();
      for (String key in keys) {
        if (key.startsWith('exam_attempt_answers_')) {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              await _notifier.cancelExamAttempt();
              await _forceClearLocalData(); // Force clear on UI side
              if (context.mounted) {
  ...
  void _showSubmitConfirmation() {
    int totalQuestions = _state.questions.length;
    int answeredCount = _state.selectedAnswers.length;
  ...
  Future<void> _submitExam() async {
    ...
    try {
      final response = await _notifier.submitExam();
      await _forceClearLocalData(); // Force clear on UI side
              ...
              children: [
                ...
                SizedBox(
                  ...
                  child: ElevatedButton(
                    onPressed: () async {
                      await _notifier.clearLocalAnswers();
                      if (context.mounted) {
                        Navigator.pop(context);
                        context.go('/exams');
                      }
      ...
      // ✅ Dù server trả 500 hay bất kỳ lỗi gì, local data vẫn phải được xóa
      // để tránh ô nhiễm dữ liệu khi người dùng làm lại bài thi.
      await _forceClearLocalData();
      if (!mounted) return;
      // Close loading dialog
      Navigator.pop(context);
    ...
    showModalBottomSheet(
      ...
      builder: (context) {
        return QuestionGridBottomSheet(
          partsInfo: _state.partsInfo,
          activeQuestionIndex: _state.activeQuestionIndex,
          selectedAnswers: _state.selectedAnswers,
          onSelectQuestion: (index) => _notifier.changeQuestion(index),
        );
      },
    );
  }
  ...
  Widget build(BuildContext context) {
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 16),
                Text(
                  'Không thể khởi tạo hoặc tải câu hỏi: ${_state.error}',
                  textAlign: TextAlign.center,
                  style: const TextStyle(color: textDark, fontSize: 16),
                ),
                const SizedBox(height: 24),
                ElevatedButton(
                  onPressed: () => _notifier.initExam(widget.examId),
                  style: ElevatedButton.styleFrom(
                    backgroundColor: primaryCobalt,
                    foregroundColor: Colors.white,
                    shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(8)),
    ...
    final activeQuestion = _state.questions[_state.activeQuestionIndex];
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text(
          _state.examTitle.isNotEmpty ? _state.examTitle : 'WT2025T21',
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    QuestionCard(
                      ...
                      isAudioPlaying: _state.isAudioPlaying,
                      audioProgress: _state.audioProgress,
                      audioCurrentSeconds: _state.audioCurrentSeconds,
                      audioTotalSeconds: ExamAttemptNotifier.audioTotalSeconds,
                      onToggleAudio: _notifier.toggleAudioPlayer,
                    ),
                    const SizedBox(height: 20),
                    ...
                    OptionsList(
                      ...
                      selectedOptionIndex: _state.selectedAnswers[_state.activeQuestionIndex],
                      onSelectOption: (idx) => _notifier.selectAnswer(idx),
                    ),
                  ],
                ),
              ),
                        ...
                        child: Container(
                          ...
                          child: Row(
                            ...
                            children: [
                              ...
                              Text(
                                _notifier.formatDuration(_state.secondsRemaining),
            ...
            Container(
              ...
              child: Row(
                ...
                children: [
                  // Prev Button
                  TextButton.icon(
                    onPressed: _state.activeQuestionIndex > 0
                        ? () => _notifier.changeQuestion(_state.activeQuestionIndex - 1)
                        : null,
                  ...
                  // Next Button
                  TextButton.icon(
                    onPressed: _state.activeQuestionIndex < totalQuestions - 1
                        ? () => _notifier.changeQuestion(_state.activeQuestionIndex + 1)
                        : null,
