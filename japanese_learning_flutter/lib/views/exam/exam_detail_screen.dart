...
class _ExamDetailScreenState extends State<ExamDetailScreen> {
  ...
  final ExamRepository _repository = ExamRepository();
  final PaymentService _paymentService = PaymentService(); // Instance service gọi API kiểm tra

  ExamDetail? _examDetail;
  bool _isLoading = true;
  bool _isUnlocked = false; // 🟢 Biến kiểm tra trạng thái mở khóa
  String? _errorMessage;

  @override
  void initState() {
    super.initState();
    _fetchDetailAndCheckAccess();
  }

  /// Tải thông tin chi tiết bài thi & kiểm tra quyền truy cập của User
  Future<void> _fetchDetailAndCheckAccess() async {
    setState(() {
      _isLoading = true;
      _errorMessage = null;
    });
    ...
    try {
      // 1. Tải chi tiết bài thi
      final detail = await _repository.getExamDetail(widget.exam.id);

      // 2. Lấy userId (Hãy đồng bộ userId này giống bên CheckoutScreen)
      final int currentUserId = 1;

      // 3. Nếu là bài thi tính phí, gọi API check xem user đã mua chưa
      bool unlocked = false;
      if (!widget.exam.isFree) {
        unlocked = await _paymentService.checkExamAccessStatus(
          examId: widget.exam.id,
          userId: currentUserId,
        );
      } else {
        unlocked = true; // Bài miễn phí thì mặc định unlocked
      }
  ...
  String _formatPrice(String price) {
    if (_isUnlocked || widget.exam.isFree) {
      return 'Miễn phí';
    }
    final lowerPrice = price.toLowerCase().trim();
    if (lowerPrice == '0' || lowerPrice == '0.0' || lowerPrice == 'miễn phí' || lowerPrice.isEmpty) {
      ...
      builder: (context) {
        return AlertDialog(
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                Navigator.pop(context); // Đóng Dialog mở khóa

                // 🟢 HỨNG KẾT QUẢ TRẢ VỀ TỪ LUỒNG THANH TOÁN (CHECKOUT -> QR)
                ...
                // 🟢 NẾU THANH TOÁN THÀNH CÔNG (NHẬN TRUE), TẢI LẠI TRẠNG THÁI BÀI THI NGAY!
                if (isSuccess == true) {
                  _fetchDetailAndCheckAccess();
                }
              },
                    ...
                    children: [
                      ...
                      Text(
                        ...
                        style: TextStyle(
                          fontSize: 20,
                          fontWeight: FontWeight.bold,
                          color: (widget.exam.isFree || _isUnlocked) ? Colors.green.shade700 : energeticOrange,
                        ),
                      ),
                    ],
                  ),
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          children: [
                            ...
                            const SizedBox(height: 8),
                            TextButton.icon(
                              onPressed: _fetchDetailAndCheckAccess,
                              icon: const Icon(Icons.refresh, size: 16, color: cobaltBlue),
                              label: const Text('Thử lại', style: TextStyle(color: cobaltBlue, fontWeight: FontWeight.bold)),
                            )
                          ],
            ...
            child: Row(
              children: [
                Column(
                  ...
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    const Text('Trạng thái', style: TextStyle(color: textLight, fontSize: 12)),
                    const SizedBox(height: 4),
                    Text(
                      (_isUnlocked || widget.exam.isFree) ? 'Đã sở hữu' : _formatPrice(widget.exam.price),
                      style: TextStyle(
                        fontSize: 16,
                        fontWeight: FontWeight.bold,
                        color: (_isUnlocked || widget.exam.isFree) ? Colors.green.shade700 : energeticOrange,
                      ),
                    ),
                  ],
                ),
                ...
                Expanded(
                  child: SizedBox(
                    ...
                    child: ElevatedButton(
                      onPressed: () async {
                        // 🟢 TỰ ĐỘNG CHUYỂN HƯỚNG DỰA TRÊN TRẠNG THÁI UNLOCKED
                        if (widget.exam.isFree || _isUnlocked) {
                          _showStartExamDialog(context);
                        } else {
                          // Nếu trực tiếp click nút MỞ KHÓA BÀI THI từ Bottom Bar
                          _showUnlockDialog(context);
                        }
                      },
                      style: ElevatedButton.styleFrom(
                        backgroundColor: (widget.exam.isFree || _isUnlocked) ? Colors.green.shade700 : energeticOrange,
                        foregroundColor: Colors.white,
                        shape: RoundedRectangleBorder(
                          borderRadius: BorderRadius.circular(8),
                        ),
                      ...
                      child: Text(
                        (widget.exam.isFree || _isUnlocked) ? 'BẮT ĐẦU THI' : 'MỞ KHÓA BÀI THI',
                        style: const TextStyle(
                          fontSize: 15,
                          fontWeight: FontWeight.bold,
                          letterSpacing: 1.1,
