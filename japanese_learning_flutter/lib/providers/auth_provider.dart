...
class AuthState {
  ...
  final String? firestoreDisplayName;
  final int? coin;
  final int? userId; // Thêm userId numeric từ Backend
...
class AuthNotifier extends AutoDisposeNotifier<AuthState> {
  ...
  Future<void> syncUserCoins() async {
    ...
    try {
      final uri = Uri.parse('${AuthService.baseUrl}/api/users/profile/${currentUser.uid}');
      final response = await http.get(uri).timeout(const Duration(seconds: 5));
      ...
      if (response.statusCode == 200) {
        ...
        if (decodedData is Map<String, dynamic>) {
          Map<String, dynamic> dataObj = decodedData;
          if (decodedData.containsKey('data')) {
            dataObj = decodedData['data'] as Map<String, dynamic>;
          }

          // Ép kiểu an toàn sử dụng num để tránh lỗi int/double
          coinValue = (dataObj['coin'] as num?)?.toInt();
          idValue = (dataObj['id'] as num?)?.toInt();
        }

        if (coinValue != null || idValue != null) {
          state = state.copyWith(
            coin: coinValue ?? state.coin,
            userId: idValue ?? state.userId,
          );
          debugPrint('✅ Đã đồng bộ xu từ BE: $coinValue');
        }
      }
    } catch (e) {
      debugPrint('❌ Lỗi đồng bộ xu từ BE: $e');
    }
  }
  ...
  Future<void> signInWithGoogle() async {
    state = state.copyWith(status: AuthStatus.loading, clearError: true);
    try {
      final fb.User? user = await ref.read(authRepositoryProvider).signInWithGoogle();
      if (user != null) {
        await ref.read(authRepositoryProvider).saveFcmToken();
        state = state.copyWith(status: AuthStatus.success, user: user);
      } else {
...
final authStateChangesProvider = StreamProvider.autoDispose<fb.User?>((ref) {
  return ref.watch(authRepositoryProvider).authStateChanges;
});
