...
class StreakNotifier extends Notifier<StreakState> {
  ...
  Future<void> performDailyCheckin(String firebaseUid) async {
    ...
    if (result != null) {
      ...
      // Mẹo nhỏ: Nếu điểm danh thành công làm tăng coin hoặc thay đổi lastLogin của User,
      // bạn có thể gọi refresh hoặc cập nhật lại State của authProvider tại đây nếu cần thiết.
      // ref.read(authProvider.notifier).updateLocalUserData(result.currentCoin, result.streakDays);
    } else {
      state = state.copyWith(
        isLoading: false,
